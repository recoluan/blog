import{_ as o,r as e,o as c,c as l,a as n,b as s,d as t,e as a}from"./app-01lAtpvS.js";const r={},u={class:"custom-container tip"},i=a('<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 24 24"><g fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"></circle><path d="M12 8h.01"></path><path d="M11 12h1v4h1"></path></g></svg><p class="custom-container-title">TIP</p>',2),k=n("li",null,[n("p",null,"最近有一个项目，考虑到要进行 SEO，所以要做成多页面应用。为了保证开发速度和开发效率，所以决定使用 webpack 做一套模块化配置方案。")],-1),d={href:"https://github.com/recoluan/webpack-multipage-demo",target:"_blank",rel:"noopener noreferrer"},m=a(`<h2 id="目录" tabindex="-1"><a class="header-anchor" href="#目录"><span>目录</span></a></h2><pre><code>|-- build                           webpack 配置
...
</code></pre><h2 id="多页面" tabindex="-1"><a class="header-anchor" href="#多页面"><span>多页面</span></a></h2><p>多页面，首先最重要的就是处理多个 html 模板和对应的多个入口文件。</p><h3 id="html-模板" tabindex="-1"><a class="header-anchor" href="#html-模板"><span>html 模板</span></a></h3><p>在项目根目录创建一个 <code>htmlarrary.js</code>，用来存储页面配置：</p><pre><code class="language-js"><span class="token comment">// htmlarrary.js</span>

module<span class="token punctuation">.</span>exports <span class="token operator">=</span> <span class="token punctuation">[</span>
  <span class="token punctuation">{</span>
    <span class="token literal-property property">_html</span><span class="token operator">:</span> <span class="token string">&#39;index&#39;</span><span class="token punctuation">,</span>
  ...
  <span class="token punctuation">{</span>
    <span class="token literal-property property">_html</span><span class="token operator">:</span> <span class="token string">&#39;login&#39;</span><span class="token punctuation">,</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">]</span>
</code></pre><p>然后在 <code>/build/utils.js</code> 创建 <code>getHtmlArray</code> 方法，用来自动生成多个模板的配置：</p><pre><code class="language-js"><span class="token comment">// /build/utils.js</span>
...
exports<span class="token punctuation">.</span><span class="token function-variable function">getHtmlArray</span> <span class="token operator">=</span> <span class="token keyword">function</span> <span class="token punctuation">(</span><span class="token parameter">moduleExportsPlugins</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
  ...
  <span class="token keyword">const</span> <span class="token function-variable function">getHtmlConfig</span> <span class="token operator">=</span> <span class="token keyword">function</span> <span class="token punctuation">(</span><span class="token parameter">name<span class="token punctuation">,</span> chunks<span class="token punctuation">,</span> title</span><span class="token punctuation">)</span> <span class="token punctuation">{</span>
    <span class="token keyword">return</span> <span class="token punctuation">{</span>
      ...
      <span class="token literal-property property">favicon</span><span class="token operator">:</span> <span class="token string">&#39;./src/assets/images/public/favicon.ico&#39;</span><span class="token punctuation">,</span>
      ...
      <span class="token literal-property property">hash</span><span class="token operator">:</span> <span class="token boolean">true</span><span class="token punctuation">,</span> <span class="token comment">// 开启hash</span>
      ...
      <span class="token literal-property property">minify</span><span class="token operator">:</span> process<span class="token punctuation">.</span>env<span class="token punctuation">.</span><span class="token constant">NODE_ENV</span> <span class="token operator">===</span> <span class="token string">&#39;development&#39;</span> <span class="token operator">?</span> <span class="token boolean">false</span> <span class="token operator">:</span> <span class="token punctuation">{</span>
        ...
        <span class="token literal-property property">collapseWhitespace</span><span class="token operator">:</span> <span class="token boolean">true</span><span class="token punctuation">,</span> <span class="token comment">// 折叠空白区域 也就是压缩代码</span>
        <span class="token literal-property property">removeAttributeQuotes</span><span class="token operator">:</span> <span class="token boolean">true</span><span class="token punctuation">,</span> <span class="token comment">// 去除属性引用</span>
      <span class="token punctuation">}</span><span class="token punctuation">,</span>
    <span class="token punctuation">}</span><span class="token punctuation">;</span>
  <span class="token punctuation">}</span><span class="token punctuation">;</span>
  ...
  <span class="token punctuation">}</span><span class="token punctuation">)</span>
<span class="token punctuation">}</span>
</code></pre><p>在 <code>webpack.base.conf.js</code> 中通过 <code>getHtmlArray</code> 添加多页面引擎配置：</p><pre><code class="language-js"><span class="token keyword">const</span> <span class="token punctuation">{</span> getHtmlArray <span class="token punctuation">}</span> <span class="token operator">=</span> <span class="token function">require</span><span class="token punctuation">(</span><span class="token string">&#39;./utils.js&#39;</span><span class="token punctuation">)</span>
...
</code></pre><h3 id="入口文件" tabindex="-1"><a class="header-anchor" href="#入口文件"><span>入口文件</span></a></h3><p>在 <code>/build/utils.js</code> 创建 <code>getEntry</code> 方法，用来自动生成入口文件的配置：</p><pre><code class="language-js"><span class="token comment">// /build/utils.js</span>
...
<span class="token punctuation">}</span>
</code></pre><p>在 <code>webpack.base.conf.js</code> 中通过 <code>getEntry</code> 添加多入口配置：</p><pre><code class="language-js"><span class="token comment">// webpack.base.conf.js</span>
...
<span class="token punctuation">}</span>
</code></pre><h2 id="js" tabindex="-1"><a class="header-anchor" href="#js"><span>JS</span></a></h2><p>JS 方面，我们一般有以下需求：</p><ol><li>eslint 错误提醒；</li><li>ts-loader 解析 typescript 语法；</li><li>babel-loader 解析 ES6 语法。</li></ol><p>针对以上需求，我们来配置一下子 rules，并且做一下延伸：</p><pre><code class="language-js"><span class="token comment">// webpack.rules.conf.js</span>

module<span class="token punctuation">.</span>exports <span class="token operator">=</span> <span class="token punctuation">[</span>
  <span class="token punctuation">{</span>
    <span class="token literal-property property">test</span><span class="token operator">:</span> <span class="token regex"><span class="token regex-delimiter">/</span><span class="token regex-source language-regex">\\.(js|ts)$</span><span class="token regex-delimiter">/</span></span><span class="token punctuation">,</span>
          ...
          <span class="token literal-property property">presets</span><span class="token operator">:</span> <span class="token punctuation">[</span>
            <span class="token punctuation">[</span><span class="token string">&#39;@babel/preset-env&#39;</span><span class="token punctuation">,</span> <span class="token punctuation">{</span>
              ...
              <span class="token literal-property property">targets</span><span class="token operator">:</span> <span class="token punctuation">{</span>
                ...
                <span class="token literal-property property">ie</span><span class="token operator">:</span> <span class="token string">&#39;8&#39;</span>
              <span class="token punctuation">}</span><span class="token punctuation">,</span>
              <span class="token literal-property property">corejs</span><span class="token operator">:</span> <span class="token number">2</span>
            <span class="token punctuation">}</span><span class="token punctuation">]</span>
          <span class="token punctuation">]</span>
        <span class="token punctuation">}</span>
      <span class="token punctuation">}</span><span class="token punctuation">,</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">]</span>
</code></pre><p>在生产环境，我们需要对 js 文件进行压缩，公共代码抽离，所以还需要在 <code>webpack.prod.conf.js</code> 中这样去优化一下：</p><pre><code class="language-js"><span class="token comment">// webpack.prod.conf.js</span>
  ...
  <span class="token literal-property property">optimization</span><span class="token operator">:</span> <span class="token punctuation">{</span>
    <span class="token literal-property property">minimizer</span><span class="token operator">:</span> <span class="token punctuation">[</span>
      ...
      <span class="token keyword">new</span> <span class="token class-name">UglifyJsPlugin</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
        <span class="token literal-property property">uglifyOptions</span><span class="token operator">:</span> <span class="token punctuation">(</span><span class="token punctuation">{</span>
    ...
    <span class="token literal-property property">splitChunks</span><span class="token operator">:</span> <span class="token punctuation">{</span>
      ...
      <span class="token literal-property property">cacheGroups</span><span class="token operator">:</span> <span class="token punctuation">{</span>
        <span class="token literal-property property">vendors</span><span class="token operator">:</span> <span class="token punctuation">{</span> <span class="token comment">// 抽离第三方插件</span>
          ...
          <span class="token literal-property property">priority</span><span class="token operator">:</span> <span class="token operator">-</span><span class="token number">10</span> <span class="token comment">// 抽取优先级</span>
        <span class="token punctuation">}</span><span class="token punctuation">,</span>
        <span class="token literal-property property">utilCommon</span><span class="token operator">:</span> <span class="token punctuation">{</span> <span class="token comment">// 抽离自定义</span>
          ...
          <span class="token literal-property property">minSize</span><span class="token operator">:</span> <span class="token number">0</span><span class="token punctuation">,</span> <span class="token comment">// 将引用模块分离成新代码文件的最小体积</span>
          <span class="token literal-property property">minChunks</span><span class="token operator">:</span> <span class="token number">2</span><span class="token punctuation">,</span> <span class="token comment">// 表示将引用模块如不同文件引用了多少次，才能分离生成新chunk</span>
...
</code></pre><h2 id="css" tabindex="-1"><a class="header-anchor" href="#css"><span>CSS</span></a></h2><p>CSS 方面，我们一般有以下需求：</p><ol><li>postcss-loader 安装 autoprefixer 插件，自动进行兼容性处理；</li><li>sass-loader 解析 sass 语法；</li><li>MiniCssExtractPlugin 进行 css 压缩。</li></ol><p>针对以上需求，我们来配置一下子 rules，并且做一下延伸：</p><pre><code class="language-js"><span class="token comment">// webpack.rules.conf.js</span>
...
module<span class="token punctuation">.</span>exports <span class="token operator">=</span> <span class="token punctuation">[</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">]</span>
</code></pre><p>在生产环境，我们需要对 css 文件进行压缩，所以还需要在 <code>webpack.prod.conf.js</code> 中这样去优化一下：</p><pre><code class="language-js"><span class="token comment">// webpack.prod.conf.js</span>
...
<span class="token keyword">const</span> prodConfig <span class="token operator">=</span> <span class="token punctuation">{</span>
  ...
  <span class="token literal-property property">plugins</span><span class="token operator">:</span> <span class="token punctuation">[</span>
    <span class="token keyword">new</span> <span class="token class-name">MiniCssExtractPlugin</span><span class="token punctuation">(</span><span class="token punctuation">{</span>
      ...
      <span class="token literal-property property">chunkFileName</span><span class="token operator">:</span> <span class="token string">&#39;[id].[contenthash:8].css&#39;</span>
    <span class="token punctuation">}</span><span class="token punctuation">)</span><span class="token punctuation">,</span>
  <span class="token punctuation">]</span>
<span class="token punctuation">}</span>

module<span class="token punctuation">.</span>exports <span class="token operator">=</span> <span class="token function">merge</span><span class="token punctuation">(</span>baseConfig<span class="token punctuation">,</span> prodConfig<span class="token punctuation">)</span>
</code></pre><h2 id="images" tabindex="-1"><a class="header-anchor" href="#images"><span>images</span></a></h2><p>images 方面，我们一般有以下需求：</p><ol><li>css 和 js 中的图片可以被解析；</li><li>html 中 img 标签的图片可以被解析。</li></ol><p>针对以上需求，我们来配置一下子 rules，并且做一下延伸：</p><pre><code class="language-js"><span class="token comment">// webpack.rules.conf.js</span>
<span class="token keyword">const</span> MiniCssExtractPlugin <span class="token operator">=</span> <span class="token function">require</span><span class="token punctuation">(</span><span class="token string">&#39;mini-css-extract-plugin&#39;</span><span class="token punctuation">)</span>
...
module<span class="token punctuation">.</span>exports <span class="token operator">=</span> <span class="token punctuation">[</span>
  ...
  <span class="token punctuation">}</span><span class="token punctuation">,</span>
  <span class="token punctuation">{</span>
    <span class="token literal-property property">test</span><span class="token operator">:</span> <span class="token regex"><span class="token regex-delimiter">/</span><span class="token regex-source language-regex">\\.(png|jpg|gif|ico)$</span><span class="token regex-delimiter">/</span></span><span class="token punctuation">,</span>
    <span class="token literal-property property">use</span><span class="token operator">:</span> <span class="token punctuation">[</span>
      <span class="token punctuation">{</span>
  ...
  <span class="token punctuation">}</span>
<span class="token punctuation">]</span>
</code></pre><h2 id="其他" tabindex="-1"><a class="header-anchor" href="#其他"><span>其他</span></a></h2><h3 id="devserver-和-热更新" tabindex="-1"><a class="header-anchor" href="#devserver-和-热更新"><span>devserver 和 热更新</span></a></h3><pre><code class="language-js"><span class="token comment">// webpack.dev.conf.js</span>
...
<span class="token keyword">const</span> devConfig <span class="token operator">=</span> <span class="token punctuation">{</span>
  <span class="token literal-property property">devServer</span><span class="token operator">:</span> <span class="token punctuation">{</span>
    ...
    <span class="token literal-property property">port</span><span class="token operator">:</span> <span class="token number">2000</span><span class="token punctuation">,</span>
    <span class="token literal-property property">useLocalIp</span><span class="token operator">:</span> <span class="token boolean">true</span><span class="token punctuation">,</span>
  ...
  <span class="token punctuation">]</span>
<span class="token punctuation">}</span>
</code></pre>`,38),g={href:"https://webpack.docschina.org/",target:"_blank",rel:"noopener noreferrer"},y=a('<h3 id="报错" tabindex="-1"><a class="header-anchor" href="#报错"><span>报错</span></a></h3><ol><li>如果 img 标签的 src 为空的话，就报错 xxxHTMLLINKxxx0.</li><li>如果报错：<code>TS2688: Cannot find type definition file for &#39;unist&#39;.</code> 说明需要安装依赖 <code>@types/unist</code>，其他类似报错一样，这是 <code>typescript@2.0</code> 更换 types 支持方式导致的报错。</li><li>编译后 css 图片路径错误，根据是否是生产环境来动态添加 <code>publicPath</code>，<a href="#CSS">点击这里</a>。</li></ol>',2);function h(f,b){const p=e("ExternalLinkIcon");return c(),l("div",null,[n("div",u,[i,n("ol",null,[k,n("li",null,[n("p",null,[s("下面主要针对一些重要的点提供思路，并不作详解。完整的代码，我会放在 github（"),n("a",d,[s("项目地址"),t(p)]),s("）上供大家参考，如果有优化的地方，请在评论区指点出来。")])])])]),m,n("blockquote",null,[n("p",null,[s("这样智能启动 css 热更新，如果需要 js 热更新，需要添加一段代码，请自行查找 "),n("a",g,[s("官网文档"),t(p)]),s("。")])]),y])}const w=o(r,[["render",h],["__file","061501.html.vue"]]),j=JSON.parse('{"path":"/blogs/frontEnd/2019/061501.html","title":"webpack 配置多页面应用的一次尝试","lang":"zh-CN","frontmatter":{"title":"webpack 配置多页面应用的一次尝试","date":"2019-06-15T00:00:00.000Z","tags":["webpack"],"categories":["frontEnd"]},"headers":[{"level":2,"title":"目录","slug":"目录","link":"#目录","children":[]},{"level":2,"title":"多页面","slug":"多页面","link":"#多页面","children":[{"level":3,"title":"html 模板","slug":"html-模板","link":"#html-模板","children":[]},{"level":3,"title":"入口文件","slug":"入口文件","link":"#入口文件","children":[]}]},{"level":2,"title":"JS","slug":"js","link":"#js","children":[]},{"level":2,"title":"CSS","slug":"css","link":"#css","children":[]},{"level":2,"title":"images","slug":"images","link":"#images","children":[]},{"level":2,"title":"其他","slug":"其他","link":"#其他","children":[{"level":3,"title":"devserver 和 热更新","slug":"devserver-和-热更新","link":"#devserver-和-热更新","children":[]},{"level":3,"title":"报错","slug":"报错","link":"#报错","children":[]}]}],"git":{"createdTime":1716921053000,"updatedTime":1716921053000,"contributors":[{"name":"Reco","email":"recoluan@qq.com","commits":1}]},"filePathRelative":"blogs/frontEnd/2019/061501.md"}');export{w as comp,j as data};
